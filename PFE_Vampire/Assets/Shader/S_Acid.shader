Shader "Custom/S_Acid"
{
	...
	#include "UnityCG.cginc"
		...
		fixed4 frag(v2f i) : SV_Target
		{
			...
			float2 uvDistort = i.worldPos * _ScaleDist;
			// moving over time
			...
			float d2 = tex2D(_DistortTex, (i.worldPos * (_ScaleDist * 0.5)) + speedDistortCombined).r;
			// combined
			float layereddist = saturate((d + d2)*0.5);

			// uv main scaled
			float2 uvMain = i.worldPos * _Scale;
			// plus distortion
